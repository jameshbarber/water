...
import { InMemoryBufferService } from "./adapters/buffer";
...
export function createDeps(manifest: AppManifest): Deps {
  ...
  const buffer = new InMemoryBufferService();
  ...
  return { logger, eventBus, rest, mcp, database, buffer, docs };
}